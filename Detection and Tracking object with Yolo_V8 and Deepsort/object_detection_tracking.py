...
video_cap = cv2.VideoCapture("people.mp4")
...
while True:
    ...
    for track in tracker.tracks:
        ...
        # If the object ID is not in the dictionary, it's a new object
            object_appearances[track_id] = {"appearance_time": datetime.datetime.now(), "disappearance_time": None}
        else:
        # If the object ID is already in the dictionary, update the disappearance time
            object_appearances[track_id]["disappearance_time"] = datetime.datetime.now()

        for object_id, timestamps in object_appearances.items():
            appearance_time = timestamps["appearance_time"]
            disappearance_time = timestamps["disappearance_time"] or datetime.datetime.now()
            object_duration = disappearance_time - appearance_time

    # Convert object_duration to seconds, minutes, or hours as needed
            seconds = object_duration.total_seconds()

    
        # draw the bounding box of the object, the name
        # of the predicted object, and the track id
        text = str(track_id) + " - " + class_name + " - " + str(seconds)
